...
- Structured codebase review sweeps (see `CODEBASE_REVIEW.md`):
  ...
  - Phase 7 (in progress): kernel/session state machine + recovery policy (docs gate complete; state-owner, status API, kernel controls, queue-pause dispatch gating, and bounded dispatch-time auto-recovery landed).
